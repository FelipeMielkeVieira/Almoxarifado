...
export class UsersService {
    ...
    retornaDisponibilidadeItem(id: number, data: Date) {
        let quantidadeTotal = this.retornaProduto(id).quantidade;
        for (const reserva of this.reservaProduto) {
            if (id == reserva.produto_id) {
                if (new Date(this.retornaReserva(reserva.reserva_id).data_retirada) < data && new Date(this.retornaReserva(reserva.reserva_id).data_devolucao) > data && !(this.retornaReserva(reserva.reserva_id).status == 2)) {
