    ...
    fimDeSemana() {
        ...
        this.monthDays.forEach(function (e) {
            if ((e.weekDayNumber == 1 || e.weekDayNumber == 7) && e.number) {
                setTimeout(() => {
                    (document.getElementById("Dia" + e.number + "Mes" + numMes + "Ano" + numAno) as HTMLElement).style.backgroundColor = "#DEDEDE";
                }, 0);
            }
        });
    }
    ...
    selecionaAutomatico() {
        if (this.anoSelecionado == this.year && this.mesSelecionado == this.monthNumber) {
            if (this.diaSelecionado != 0 && this.mesSelecionado != 0 && this.anoSelecionado != 0) {
                setTimeout(() => {
                    (document.getElementById("Dia" + this.diaSelecionado + "Mes" + this.mesSelecionado + "Ano" + this.anoSelecionado) as HTMLElement).style.backgroundColor = '#5E88CB';
                }, 0);
            }
        }
    }
